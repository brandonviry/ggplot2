...
Dans ce guide nous allons vous accompagner et vous introduire à l'utilisation de ce package ,malgré l'aide fournis il faudra préalablement des **prérequis** afin de facilement comprendre et suivre ce tuto de façon la plus optimal. Ensuite vous verrez les différentes **syntaxe** , nous vous donnerons 2 exemple de cas d'utilisation et pour **conclure** on vous donnera les **ressources** nécessaire pour aller plus loin dans votre **apprentissage**.

Malgré l'effort et le travail fournit pour vous donnez un guide de qualité on arrivera pas à concurrencer votre meilleur allié qui sera la fonction **" help()"** qui vous sera quasiment indispensable dans votre apprentissage !!!
...
Pour pouvoir utiliser ce guide **il faut maîtriser le langage de programmation R** il faut au préalablement avoir installé sur sa machine **r studio** , installer la librairie  **ggplot2** et savoir **le type de représentation** qu'on veut faire sur vos données .Si vous avez jamais installer une librairie , utiliser R et Rstudio je vous met en lien un [tutoriel](https://openclassrooms.com/fr/courses/4525256-initiez-vous-au-langage-r-pour-analyser-vos-donnees/6250851-prenez-en-main-r-et-r-studio) expliquant comment utiliser R et Rstudio et je vais vous expliquer comment installer la librairie ggplot2 dans ce guide .
...
## Récupération de données (Brandon)

Au vue de la quantité exponentielle des données , celle-ci peuvent entre être enregistrées  **dans des fichiers**  et le plus souvent ce fichier est dans le format csv  donc je vais vous montrer comment récupérer ces données  dans  plusieurs types de fichier . Ces données sont representer dans le langage R **sous forme de** **Dataframe** .Un dataframe est un tableau de **données** ou chaque colonnes sont **typées** et ou les noms des colonnes sont appelé variable . Voici un exemple :
...
**data :** La variable où les données vont être sauvegarder (le tableau).

**read.table() :** La fonction qui prend en argument :

1.  **le nom du fichier** pour l'exemple c'est data.txt sous forme de chaîne de caractère c'est à dire entre guillemet "data.txt".
...
**read.csv() :** Cette fonction est réservé au fichier en **format .csv** et prend en argument :

1.  **Le nom du fichier** pour l'exemple c'est tic-tac-toe.csv sous forme de chaîne de caractère c'est à dire entre guillemet "tic-tac-toe.csv".
...
Après avoir récupérer les données vous devez d'abord **importer la librairie** ggplot2 pour faire la représentation :

```{r}
library(ggplot2)
```

Pouvoir travailler sur ces données vous devez **prendre ces données est les placer en argument dans la fonction ggplot** comme ceci  :
...
Vous vous demandez surement pourquoi il y'à un carré gris sans graphique , c'est tout à fait normal car on n'a pas encore défini de Géométrie . C'est le prochain point de ce guide .

## Les Géométries (Brandon)

**Les géométries ,appeleé aussi GEOM sont les types de représentation graphique** qu' on veut utiliser sur nos données  il existe beaucoup  de géométrie  chaque géométrie à ces caractéristique . 

Pour **sélectionner** la représentation qu'on veut il suffit **d' ajouter un type de graphique**   qui est sous forme de fonction :
...
Comme vous le voyez **les géométries ne peuvent être représenter sans un argument** .

**Les géométriques** peuvent avoir en **argument** ce qu' on appelle un **mappage** et **des attribue graphique** (qui est le prochain point ) . Un **mappages** permet de **relier un élément graphique à une variable** d'un tableau.Il existe presque autant de mappage que il y de attribue graphique. Pour chaque géométrie il existe différent mappage . Ces **mappages** sont défini dans la fonction **aes**() cette fonction permet de **sélectionner les éléments à utiliser pour la représentation selon les caractéristiques** d'un type de graphique .Certains **mappages sont obligatoire** pour faire une représentation et d' **autres sont optionnelle** .
...
-   **x** : Un élément du mappage , c'est l'élément du graphique en x ,

-   **y** : Un élément du mappage ,c'est l'élément du graphique en y ,

-   **data1** : La donnée pris en compte en x ,

-   **data2** : La donnée pris en compte en y .

Il existe beaucoup de  géométrique  mais on va vous présenter quelques un  avec les mappage obligatoire .
...
**RQ** : geom_violin et geom_boxplot sont des façon de visualiser très similaires .
...
**RQ** : coord_flip() s'applique à toute les type de représentation de ggplot 2.\
...
**RQ**: geom_text  et geom_label sont identique sauf dans l'affichage du texte .
...
**RQ** : geom_point, geom_hexa ,geom_bin2d leurs différence se situe dans la forme géométrique de la représentation : geom_point c'est un point ,geom_hexa  c'est un hexagone ,geom_bin2d un polygone à 4 côtés .

Il  en existe d'autres mais voici les principaux utiliser.

<https://bookdown.org/ael/rexplor/chap8.html#geom>

## Les mappage optionnelle (Brandon)

Les **mappages** optionnels sont les **mêmes** que les **Attributs de représentation** sauf qu' ils agissent selon la variable qu' on lui passe en paramètre .
...
**size** : Un mappage optionnelle .

```{r}
ggplot(note)+geom_point(aes(y=COUR,x=Note ,color=ECTS)) 
```

**color** : Un mappage optionnelle .
...
## Attribut de représentation (Beriche)

Les  éléments de **représentation graphique**  appelés **GEOM** , utilise differente propriété passé en paramètre pour représenter des variables , dans un ggplot, le terme "**esthétique"** fait référence aux éléments de style utilisés pour visualiser des variables du **data.frame** de travail et non à l'affichage des **éléments externes** (non-data), tels que les titres, les étiquettes des axes, la **couleur** d'arrière-plan, etc., il faut . savoir que les **arguments estetiques** qu'on uttilise dans un ggplot dépendent des geoms(point,lines,bars,etc...)

[**Voici une liste non exhaustives des propriété qui sont partagé par plusieurs geoms :**]{.underline}

-   **y :** Variable numérique dont on veut étudier la répartition .  ,
...
-   **size :** La Taille des symboles.

-   **alpha :** La Transparence.

-   **fill :** La couleur du remplissage.

-   **group :** Tracer des éléments séparément pour chaque groupe.

Il existe plusieurs propriétés pour personnaliser son graphe mais on ne pourra pas tous les citer ici , voici quelques ressources pour allez plus loin et en savoir plus sur celles-ci, pour cela consultez son help. Exemple : **help(geom_NOMGEOM)** ou **?geom_NOMGEOM**
...
Pour notre exemple ici on va prendre comme donnée source en **abscisse** x le cylindrée du moteur en litre avec "displ"  et en **ordonnée** y le modèle de véhicule concernée "model"

```{r}
#Voici 2 exemple avec differrent geom
#dans cette exemple on crée un geom de type point et on lui passe quelque argument tel que la taille des symbole,sa couleur
...
L'un des avantages de ggplot c'est qu'on peut combiner plusieurs types de graphes ensemble, pouvant être de type et de sources de données différentes tous simplement avec le symbole +.

Voici un exemple :

```{r}
#  je vais combiner les deux graphe de ligne et de point  precedent
...
Comme vue précedemment on va prendre l'habitude d'utiliser help quand on ne connait pas une fonction ou comment elle fonctionne.
...
Ces deux fonctions sont très similaires mais **facet_grid()** réalise une matrice de graphiques alors que **facet_wrap()** réalise une suite de graphiques. Les deux arguments principaux de ces fonctions sont : **rows** et **cols** pour **facet_grid()** et **facets** pour **facet_wrap()**. Les graphiques suivants illustres ces deux fonctions et leurs arguments.
...
Les scale ou **échelles** permettent de **déterminer** la façon dont **un graphique affiche les données** avec les attributs graphiques et les mappages .
...
La première **fonction** scal que je vais vous présentez permet de **modifier la taille  des éléments  à représenté** cette fonction se nomme scale_size cette fonction **modifie minimales et maximales  de la taille** de l'objet sur un mappage size cette fonction prend en argument **un argument range d'un vecteur de taille 2 avec le min et le max** .
...
Les prochaines fonctions sont les scale qui modifient sur les X et Y 
...
Les fonctions du genre s**cale_x.... ou scale_y...**  Modifie **l'axe x et y du graphique** , ces fonctions prennent en argument, **un argument limite** qui est un vecteur de t**aille 2 avec le min et le max .** 
...
Il existe une autre type de **scal** qui permet de **modifier la couleur** ,ces fonctions sont du genre scale_color ...   qui modifie **les palettes de couleur**  lorsqu'on utilise l'attribut graphique  **color** .
...
-   **scale_color_gradient** qui prend en argument deux paramètre low et high ou low et higt sont une couleur en chaîne de caractère ,low représente les valeurs les plus faibles et high représente les valeurs les plus fortes de la variable représenter .

-   **scale_color_viridis_c** : ajoute un titre à la légende couleur .

-    **scale_fill** :  modifie les palettes de couleur  lorsqu'on utilise l'attribut graphique fill

## Theme (Beriche)

Nous voici arriver sur une partie assez amusante de ce guide les thèmes . Les thèmes permettent de contrôler et personnaliser l'affichage de tous les éléments du graphique qui ne sont pas reliés aux données : titres, étiquette , grilles, fonds, légende et bien plus .

Il existe un certain nombre de thèmes préexistants ici on va utiliser deux thème sur notre exemple :
...
Un autre exemple de thème **theme_minimal** :
...
Les éléments personnalisables étant nombreux, un bon moyen de se familiariser avec tous les arguments est sans doute le menu Addins de RStudio après avoir au préalablement installer **ggThemeAssist**. Pour ne pas trop se disperser, je vais vous donnez un lien menant vers un tuto qui vous apprendra de façons simple à utiliser ggTemeAssist <https://www.dataindeed.io/2018/03/13/dailyr-ggplot2-theme-assistant/>.

Les thème permettent d'obtenir très facilement des rendu graphique beau mais si mal utiliser peuvent aussi donner des graphe pas très esthétiques et moins compréhensible , dans l'exemple ci-dessous  je fais exprès de mélange beaucoup de thème, ce qui va donner un résultat pas très beau et peu compréhensible , c'est pour cela que je vous ai présenté plus haut deux thème simple pour vous habituez à choisir des options simple et compréhensible plutôt que des options qui peuvent rendre le graphe illisible .

```{r}
#Exemple quand on melange beaucoup d'options sans les maitriser
...
Iici on vous a donner 2 exemple de thème et un moyen de les modifier , mais il existe des centaines de thème permettant de donner un beau rendu visuel à vos graphiques , vous pourrez trouver une panoplie de thème à l'aide de :
...
Il y' a aussi un package complémentaire dédié au thème appelé ggthemes.
...
## Exemple d'application du guide (Beriche)
...
Maintenant que j'ai préparé les données je vais utiliser ggplot pour visualiser les données, J'utilise le Geom Point qui est assez simple à mettre en œuvre et pour que vous puissiez comprendre plus facilement. Mais vous pouvez utiliser le geom de votre choix ,celui que vous maîtrisez le mieux par exemple. Mais n'ayez crainte peut importe le geom et les donne choisit vous saurez les maîtriser grâce à notre guide.
...
```{r}
ggplot(donnee,aes(x=Matiere,y=Notes)) + geom_point(alpha=2,shape=18,size=3,color="blue",fill=3) + geom_line(aes(y=NotesPrec),color="red",alpha=1,linetype=4,linewidth=2,group=2)
```

J'ai choisi de représenter les moyenne des notes de l'année précédente en geom_line ,couleur rouge, Par exemple dans ce graphe on peut apercevoir que la plupart des Notes moyenne par matière était basse sauf pour Python, ou la moyenne de cette année à chuter par à port à la promo de l'année dernière.

## Facetting

Maintenant on va découper le graphe en plusieurs sous graphiques séparément pour pouvoir isoler et interpréter chaque Notes et matière différemment, grâce au facetting , ici j'ai choisi de les disposer par suite graphique grâce à la fonction facet_wrap.

```{r}
#help("facet_wrap")
ggplot(donnee,aes(x=Matiere,y=Notes)) + geom_point(alpha=2,shape=18,size=3,color="blue",fill=3) +
  facet_wrap(facets = vars(Matiere, Notes))

```

## Theme

Je vais finir mon exemple en personnalisant mon graphe initial pour le rendre plus beau visuellement grâce au thème .

```{r}
help("theme")
ggplot(donnee,aes(x=Matiere,y=Notes)) + geom_point(alpha=2,shape=18,size=3,color="blue",fill=3) + theme( 
  
  #Je vais changer les legendes en rouges,gras,police en model serif
  axis.title = element_text(family = "serif",
    size = 18, face = "bold", colour = "red"),
  
  #Je vais mettre le fond en vert
  plot.background = element_rect(fill = "green"),
  
  #je change la taille et le format des différrente matieres et notes
  axis.text = element_text(family = "serif",
        size = 12, face = "bold"), axis.text.x = element_text(family = "mono"),
  
  #Je change la couleur du  fond du graphique
   panel.background = element_rect(fill = "coral")
  
  )
```

Pour mon exemple , j'ai suivi les indications du guide ,qui dit qu'il ne faut pas mélanger trop d'attributs pour personnaliser le graphe afin d'éviter qu'il soit illisible et incompréhensible.

Maintenant c'est à votre tour de jouer, Pour vous entrainez choisissez des données à visualiser en utilisant le guide comme je les fait précédemment . Et envoyez-nous votre projet . Merci pour votre participation.

## Exemple d'utilisation de ggplot 2 (Brandon)

Bon voici un exemple d' utilisation de ggplot2 d'un cas pratique .Dans cet exemple on veut visualiser les pourcentages des variations entre l'euro et le dollar .

Premièrement je récupère les données. J'ai choisi de prendre les données depuis un fichier .csv
...
Après avoir récupérer les données , je veux les visualisées avec des couleurs , j'ai importer des couleurs possible pour la représentation de mon graphique .
...
Maintenant que j'ai mes couleurs et mes données je peut commencer la visualisation.en important la librairie GGPLOT 2.

```{r}
library(ggplot2)
```

Je commence à crée la visualisation sur mes données :
...
Plusieurs points sont traités en référence au guide comme l'ajout de graphe multiple ,des scales , modification de thème ,des mappage obligatoire et optionnel ,plusieurs géométrie , des attributs de représentation .

# Aller Plus loin (Bériche et Brandon)
...
Il est temps de se dire au revoir après tant de chemins parcourus ensemble dans ce tutoriel, vous voilà maintenant bien équipé avec assez de base  pour vous lancer seul de manière plus approfondie dans  ce vaste monde qu'est la visualisation des données avec ggplot2 , mais ne vous inquiétez pas on va vous tenir la main une dernière fois en vous donnant quelques idées et ressources(voir plus haut) pour approfondir votre apprentissage, un dernier conseil n'hésitez pas à commenter votre code cela pourrait vous être beaucoup utile plus tard et surtout pour la compréhension de votre code . Merci d'avoir suivi notre guide et à très bientôt.

### [Voici quelques sujet très intéressant,vous pourriez :]{.underline}

-   **Ecrives vos propres fonctions**
